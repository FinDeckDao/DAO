...
export const NavBar = () => {
  ...
  const getLoginButton = (auth: AuthContextType) => {
    ...
    if (!auth.isAuthenticated) {
      return <Link
        to="/dashboard"
        className="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-sky-100 hover:bg-black"
      >
  ...
  return <header className="col-span-12 bg-[#0d0d0d] text-sky-100 rounded-b-lg">
    {/* Default Menu */}
    <nav className="mx-auto flex items-center justify-between p-4 lg:px-8" aria-label="Global">
      <div className="flex lg:flex-1">
        <Link to="/" className="text-sky-100 font-bold hover:bg-black block rounded-lg px-3 py-2.5">
          dao.FinDeck.io
        </Link>
      </div>
      ...
      <div className="hidden lg:flex lg:gap-x-12">
        {navigation.map((item) => (
          <Link
            key={item.name}
            to={item.href}
            className="text-sm font-semibold leading-6 text-sky-100 hover:bg-black rounded-lg p-3"
          >
            {item?.icon} {item.name}
          </Link>
        ))}
            ...
            <div className="space-y-2 py-6">
              {navigation.sort((a, b) => a.name.localeCompare(b.name)).map((item) => (
                <Link
                  ...
                  to={item.href}
                  onClick={() => setMobileMenuOpen(false)}
                  className="-mx-3 block rounded-lg px-3 py-2 text-base leading-7 text-sky-100 hover:bg-black uppercase"
                >
                  {item.name}
                </Link>
              ))}
